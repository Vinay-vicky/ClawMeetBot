...
export default function Dashboard() {
  ...
  const subtitle = <><span className="live-dot" />{' '}Updated: <span id="ts">{nowStr}</span></>
  const navExtra = (
    <>
      <span className="countdown">Auto-refresh in <b style={{ color:'#e1e4e8' }}>{countdown}s</b></span>
  ...
  return (
    <Layout subtitle={subtitle} navExtra={navExtra}>
      ...
      <div className="fc" style={{ background:'linear-gradient(135deg,#161b22 0%,#1c2128 100%)', borderColor:'#1f6feb', textAlign:'center', padding:'28px 20px' }}>
        <div style={{ fontSize:36, marginBottom:10 }}>📊</div>
        <div style={{ fontSize:16, fontWeight:600, color:'#58a6ff', marginBottom:8 }}>Team Analytics</div>
        <p style={{ color:'#8b949e', fontSize:12, marginBottom:18, lineHeight:1.7 }}>
        ...
        <Link to="/analytics" style={{ display:'inline-block', background:'#1f6feb', color:'#fff', padding:'10px 28px', borderRadius:6, textDecoration:'none', fontSize:13, fontWeight:600 }}>
          📊 View Analytics &rarr;
        </Link>
      </div>
      ...
      {/* Pending Tasks */}
      <div className="fc">
        <h2>✅ Pending Tasks <span style={{ color:'#484f58', fontWeight:400, textTransform:'none', letterSpacing:0 }}>(top 30 — click ✅ to complete)</span></h2>
        <div className="table-scroll">
          <table>
