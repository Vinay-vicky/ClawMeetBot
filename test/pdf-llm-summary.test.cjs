...
test('generateLlmContextVariants creates structured fallback summaries without AI keys', async () => {
  ...
  assert.ok(variants.medium.includes('Summary') || variants.medium.includes('Overview'))
  assert.ok(variants.small.includes('Quick LLM Brief') || variants.small.includes('Core Themes') || variants.small.includes('Key Points'))
